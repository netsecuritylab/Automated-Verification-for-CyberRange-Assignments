...
def execute_command_ssh(hostname, username, password, command,sshcopykey_command, owner_command1, owner_command2, authorized_key_command):
    ...
    try:
        ...
        #Comando se non generi le chiavi per tutti: 
    ...
    #Preparati gli utenti tester, andiamo ora a dargli la versione privata della chiave per i test di SSH annidata
    """
    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    
    try:
        client.connect(hostname=hostname, username="tester", key_filename='SSHKey')  
        ...
        try:
            ...
            print(f"Failed to open SFTP channel: {e}")
    ...
    Chiave_SSH = "SSHKey"
    remote_path = "/home/tester/.ssh/"
    username = "tester"
    File_da_Copiare= "SSHKey"
    
    # Costruisci il comando come stringa
    command = f"scp -i {Chiave_SSH} {File_da_Copiare} {username}@{hostname}:{remote_path}"
    
    # Esegui il comando utilizzando subprocess
    try:
        subprocess.run(command, shell=True, check=True)
        
        # Imposta i permessi del file copiato a 600
        chmod_command = f"ssh -i {Chiave_SSH} {username}@{hostname} chmod 600 {remote_path}/{File_da_Copiare}"
        subprocess.run(chmod_command, shell=True, check=True)
        
        print(hostname, "-->enviroment di testing creato!")
    except subprocess.CalledProcessError as e:
        print(f"Si è verificato un errore durante la copia della chiave privata sull'host': {e}")



# Controlla se il file "SSHKey" esiste nella cartella corrente
if not os.path.exists("SSHKey"):
    print("Il file 'SSHKey' non esiste nella cartella corrente. Uscita in corso...")
    sys.exit()
